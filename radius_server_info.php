...
// 표시될 이름 => 실제 pgrep으로 검색할 프로세스명
$services_to_check = array(
    "FreeRADIUS" => "radiusd",   // Ubuntu/Debian은 "freeradius"
    "MySQL"      => "mysqld",
    "SSHd"       => "sshd"
);
...
foreach ($services_to_check as $label_name => $process_name) {
    $running = check_service($process_name);
    $class = ($running) ? "text-success" : "text-danger";
    $label = ($running) ? "running" : "not running";
    
    $value = sprintf('<span class="%s">%s</span>', $class, $label);
    
    $table['rows'][] = array($label_name, $value);
}
