...
read -p "Enter MySQL/MariaDB Host (Enter 입력시 : localhost): " input_host
MYSQL_HOST=${input_host:-"localhost"}
read -p "Enter MySQL/MariaDB Port (Enter 입력시 : 3306): " input_port
MYSQL_PORT=${input_port:-"3306"}
read -p "Enter MySQL/MariaDB Database (Enter 입력시 : radius): " input_db
MYSQL_DATABASE=${input_db:-"radius"}
# --- 3-3. daloRadius에서 사용할 DB에 사용자 설정 ---
echo "--- 3-3. daloRadius에서 사용할 DB에 사용자 설정..."
read -p "Enter MySQL/MariaDB User (Enter 입력시 : radius): " input_user
MYSQL_USER=${input_user:-"radius"}
read -s -p "Enter MySQL/MariaDB Password (Enter 입력시 : radius12#$): " input_pw
...
####sed -i "s/\$configValues\['CONFIG_DB_ENGINE'\] = '.*';/\$configValues\['CONFIG_DB_ENGINE'\] = 'mysqli';/" "${WEB_ROOT}/radius/app/common/includes/daloradius.conf.php"
sudo sed -i "s/\$configValues\['CONFIG_DB_HOST'\] = '.*';/\$configValues\['CONFIG_DB_HOST'\] = '${MYSQL_HOST}';/" "${WEB_ROOT}/radius/app/common/includes/daloradius.conf.php"
sudo sed -i "s/\$configValues\['CONFIG_DB_USER'\] = '.*';/\$configValues\['CONFIG_DB_USER'\] = '${MYSQL_USER}';/" "${WEB_ROOT}/radius/app/common/includes/daloradius.conf.php"
sudo sed -i "s/\$configValues\['CONFIG_DB_PASS'\] = '.*';/\$configValues\['CONFIG_DB_PASS'\] = '${MYSQL_PASSWORD}';/" "${WEB_ROOT}/radius/app/common/includes/daloradius.conf.php"
...
# --- 8. 서비스 시작 및 방화벽 설정 ---
# --- 8-1. 웹, Radius 서비스 시작 ---
sudo rm -f /etc/httpd/conf.d/daloradius-users.conf
sudo rm -f /etc/httpd/conf.d/daloradius-operators.conf
